(function () {
  ...
  // IMPORTANT: Login/OTP must NEVER auto-open on page load.
  // It is opened only when the customer explicitly clicks the account/login control.
  // Ezy Returns is fully isolated as it has its own authentication/form flow.
  ...
  function openLogin() {
    ...
    try {
      ...
      // Fallback for themes/apps that expose the login dialog through the DOM.
  ...
  // Manual trigger only. No timers, retry loops, mouse/scroll wake-up,
  // 8-second popup, 50-second popup, or 120-second popup remain.
  ...
  console.log('TVASTRA LUCENT: manual-login mode enabled; no auto popup.');
}());
